...
import com.course.dto.CourseGraphDto
import com.course.dto.CreateCourseDto
import com.course.dto.EdgeDto
import com.course.dto.StepDetailDto
import com.course.dto.StepDto
import com.course.dto.UpdateCourseDto
import com.course.dto.UpdateStepDto
...
import com.shared.exception.ValidationException
...
class CourseService(
    ...
    fun createCourse(dto: CreateCourseDto, ownerId: Long): Course {
        ...
        val owner = userRepository.findById(ownerId)
            .orElseThrow { NotFoundException(ownerId, "Пользователь") }
    ...
    fun getCourseDto(courseId: Long, userId: Long): CourseDto {
        ...
        if (!canView(course, userId)) {
            throw AccessDeniedException("Нет прав для просмотра курса")
        }
        ...
        return CourseDto(
            ...
            owner = UserSummaryDto.Companion.from(course.owner),
    ...
    fun updateCourse(courseId: Long, dto: UpdateCourseDto, requesterId: Long): Course {
        ...
        if (!canEdit(course, requesterId)) {
            throw AccessDeniedException("Нет прав для изменения курса")
        }
    ...
    fun deleteCourse(courseId: Long, requesterId: Long) {
        ...
        if ((course.owner.id != requesterId) && (course.memberships.find { it.user.id == requesterId }?.ability != UserAbility.ADMIN)) {
            throw AccessDeniedException("Нет прав для удаления курса")
        }
    ...
    fun startCourse(courseId: Long, userId: Long): CourseEnrollment {
        ...
        if (course.enrollments.any { it.user.id == userId }) {
            throw AlreadyExistsException("Уже записан")
        }
    ...
    fun addMember(courseId: Long, userId: Long, ability: UserAbility, granterId: Long): CourseMembership {
        ...
        if (!canManageMembers(course, granterId)) {
            throw AccessDeniedException("Нет прав для добавления пользователей")
        }
        if (course.owner.id == userId) {
            throw AlreadyExistsException("Пользователь уже является участником (владельцем)")
        }
        if (course.memberships.any { it.user.id == userId }) {
            throw AlreadyExistsException("Пользователь уже является участником")
        }
        val user = userRepository.findById(userId)
            .orElseThrow {
                NotFoundException(
                    userId,
                    "Пользователь",
                    "Пользователь для добавления не найден (id=$userId)"
                )
            }
        val granter = userRepository.findById(granterId)
            .orElseThrow { NotFoundException(granterId, "Админ") }
    ...
    fun removeMember(courseId: Long, userId: Long, removerId: Long) {
        ...
        if (!canManageMembers(course, removerId)) {
            throw AccessDeniedException("Нет прав для удаления участников")
        }

        if (course.owner.id == userId) {
            throw AccessDeniedException("Нельзя удалить владельца курса")
        }

        if (userId == removerId) {
            course.memberships.find {
                it.user.id != removerId && it.ability == UserAbility.ADMIN
            }?:throw AccessDeniedException("Нельзя удалить себя — вы последний администратор")
        }
    ...
    fun updateMemberAbility(
        ...
        if (!canManageMembers(course, updaterId)) {
            throw AccessDeniedException("Нет прав для изменения участников")
        }
        ...
        val membership = course.memberships.find { it.user.id == userId }
            ?: throw NotFoundException(
                userId,
                "Участник",
                "Участник не найден в курсе (участник: id=$userId, курс: id=$courseId)"
            )

        if (course.owner.id == userId) {
            throw AccessDeniedException("Нельзя изменить права владельца курса")
        }
        ...
        if (membership.user.id == updaterId && newAbility != UserAbility.ADMIN) {
            if (userId == updaterId) {
                ...
                }?:throw AccessDeniedException("Нельзя понизить себя — вы последний администратор")
            }
    ...
    fun getCourseGraph(courseId: Long, requesterId: Long): CourseGraphDto {
        ...
        return CourseGraphDto(
            steps = (course.steps).map { step -> StepDto.Companion.from(step) },
            edges = (course.edges).map { edge -> EdgeDto.Companion.from(edge) }
        )
    }
    @Transactional
    fun updateCourseGraph(courseId: Long, dto: CourseGraphDto, requesterId: Long)  {
        ...
        if (!canEdit(course, requesterId)) {
            throw AccessDeniedException("Нет прав для изменение курса")
        }
    ...
    fun getStep(courseId: Long, stepId: Long, userId: Long): StepDetailDto {
        ...
        val step = stepRepository.findById(stepId)
            .orElseThrow { NotFoundException(stepId, "Шаг") }
        if (step.course.id != courseId) {
            throw ValidationException("Шаг не принадлежит курсу")
        }
