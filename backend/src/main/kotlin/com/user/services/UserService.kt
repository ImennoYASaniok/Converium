...
class UserService(
    ...
    fun updateUser(id: Long, request: UpdateUserRequest, currentUserId: Long): UserDto {
        ...
        request.login?.let { newLogin ->
            require(!userRepository.existsByLogin(newLogin) || user.login == newLogin) {
        ...
        request.email?.let { newEmail ->
            require(!userRepository.existsByEmail(newEmail) || user.email == newEmail) {
                "Email уже используется"
            }
            user.email = newEmail
        }
        request.theme?.let { newTheme -> user.theme = normalizeAndValidateTheme(newTheme) }
    ...
    fun sendFriendRequest(fromUserId: Long, toUserId: Long, currentUserId: Long) {
        require(currentUserId == fromUserId) {
            "Отправка заявки можно делать только со своего пользователя"
        }
        ...
        require(!userRepository.areFriends(fromUserId, toUser.id)) {
            "Этот пользователь уже добавлен в друзья"
        }
        require(!userRepository.hasFriendRequest(toUserId, fromUser)) {
            "Запрос в друзья для этоого пользователя уже отправлен"
        }
        require(!userRepository.hasFriendRequest(fromUserId, toUser)) {
            "Этот пользователь уже отправил вам запрос в друзья"
        }
    ...
    fun acceptFriendRequest(acceptorUserId: Long, requesterId: Long, currentUserId: Long) {
        require(currentUserId == acceptorUserId) {
            "Приём заявки в друзья можно делать только со своего пользователя"
        }

        val acceptor = getUserById(acceptorUserId)
        val requester = getUserById(requesterId)

        require(userRepository.hasFriendRequest(acceptorUserId, requester)) {
            "Нет запросов в друзья у отправителя"
        }
    ...
    fun rejectFriendRequest(rejectorUserId: Long, requesterId: Long, currentUserId: Long) {
        require(currentUserId == rejectorUserId) {
            "Вы можете отклонять заявки только с вашего аккаунта"
        }

        val rejector = getUserById(rejectorUserId)
        val requester = getUserById(requesterId)

        require(userRepository.hasFriendRequest(rejectorUserId, requester)) {
            "Нет запросов для этого пользователя на отклонение"
        }
    ...
    fun cancelFriendRequest(cancelerUserId: Long, targetUserId: Long, currentUserId: Long) {
        require(currentUserId == cancelerUserId) {
            "Вы можете отменить заявки только с вашего аккаунта"
        }

        val canceler = getUserById(cancelerUserId)
        val target = getUserById(targetUserId)

        require(userRepository.hasFriendRequest(targetUserId, canceler)) {
            "Нет заявок для отмены на этого пользователя"
        }
    ...
    fun removeFriend(userId: Long, friendId: Long, currentUserId: Long) {
        ...
        require(userRepository.areFriends(userId, friend.id)) {
            "Вы не можете удалить этого пользователя из друзей, так как он изначально у вас не в друзьях"
        }
    ...
    fun getFriends(userId: Long, currentUserId: Long?): List<FriendDto> {
        ...
        return friends.map { friend -> FriendDto(user = buildUserDto(friend, currentUserId)) }
    }

    fun getIncomingFriendRequests(userId: Long, currentUserId: Long?): List<FriendRequestDto> {
        require(currentUserId == userId) {
            "Вы можете увидеть свои запросы в друзья только с вашего аккаунта"
        }
    ...
    fun banUser(userId: Long, bannedUserId: Long, currentUserId: Long) {
        require(currentUserId == userId) {
            "Вы можете банить пользователей только с вашего аккаунта"
        }
        ...
        require(!userRepository.isBanned(userId, bannedUser)) {
            "Этот пользователь уже вами забанен"
        }
    ...
    fun unbanUser(userId: Long, bannedUserId: Long, currentUserId: Long) {
        require(currentUserId == userId) {
            "Вы можете разбанить пользователя только с вашего аккаунта"
        }
    ...
    fun getBannedUsers(userId: Long, currentUserId: Long?): List<BannedUserDto> {
        require(currentUserId == userId) {
            "Вы можете посмотреть список забанненых пользователей только с вашего аккаунта"
        }
    ...
    private fun buildUserDto(user: User, currentUserId: Long?): UserDto {
        ...
        val hasIncomingRequest =
                currentUserId != null &&
                        userRepository.hasFriendRequest(userId, getUserById(currentUserId))
        val hasOutgoingRequest =
                currentUserId != null && userRepository.hasFriendRequest(currentUserId, user)
